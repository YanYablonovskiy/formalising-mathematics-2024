...
example : ¬(P ∧ Q) ↔ ¬P ∨ ¬Q :=
  ⟨fun nP_Q ↦ (Classical.em P).elim (fun hp ↦ (Classical.em Q).elim (fun hq ↦ (nP_Q ⟨hp,hq⟩).elim) (fun nq ↦ Or.inr nq))  (fun np ↦ Or.inl np),
     fun nPoQ ↦ nPoQ.elim (fun np ↦ (fun np_q ↦ np np_q.1)) (fun nq ↦ (fun np_q ↦ nq np_q.2))⟩
